...
describe('Delete feature flag e2e tests', () => {
  ...
  it('should delete a feature flag', async () => {
    ...
    await request(app.getHttpServer())
      .delete(`/feature-flag/${featureFlagCreated.id}`)
      .set('Authorization', authToken)
      .expect(204);

    await expect(
      featureFlagRepository.findById(featureFlagCreated.id),
    ).rejects.toThrow();
  });
